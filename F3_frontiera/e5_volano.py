...
# -*- coding: utf-8 -*-
"""
e5_volano.py v0.1 - CANTIERE E5 ("il vero E4" di MIKE) (bonus finale, fuori par.12, dichiarato):
JUMP-TO-STEP + VOLANO BRACCIA/VITA: il salto di E4 piu' l'organo che mancava
(scoperta 27): le braccia frustano all'indietro al primo appoggio per bere il
momento angolare del tronco (assorbimento rapido, ritorno lento), la vita
assiste. Attivo anche in spinta per pulire il decollo alla fonte.

    python e5_volano.py            demo 3D: salta e atterra coi passi (loop)
    python e5_volano.py --test     misura: volo + passi di recupero + esito

STATO DEL CANTIERE (onesto): il VOLANO FUNZIONA - frustata a 40 Nm per la
corsa intera, vx da -0,67 a 0 in 80 ms, decollo ripulito fino a -0,06; NOVA
resta IN PIEDI PER 0,5 s dopo il salto (z 0,87, busto -3 gradi), poi deriva.
Scoperta 28: braccia dietro post-frustata = CoM -3 cm = la deriva; fermata e
passo si contendono lo stato: serve un SUPERVISORE (eventuale E6).
NON ancora ferma-indefinitamente. kp gambe x3 [A].
"""
import os as _os
def _MP(_n):
...
# --- VOLANO braccia/vita (scoperta 27) ---
Q_PRONTE = -1.0           # braccia avanti in volo: caricate per la frustata
K1F, K2F = 4.0, 2.5       # frustata: proporzionale a (P0R - pitch) e a -wy
RATE_ASSORBI, RATE_RITORNO = 14.0, 2.5   # rad/s: assorbe veloce, ricicla
GOMITO = 0.15             # braccia quasi tese = piu' inerzia di volano
T_MIN_R = 0.15
...
class Ginnasta:
    ...
    def reset(self):
        ...
        self.caduta = False
        self.q_fw = 0.0           # comando corrente del volano (spalle)
        self.calmo = 0            # frame consecutivi dentro la scatola di presa
    ...
    def frame(self):
        ...
        # ---------- doppio appoggio finale ----------
            ...
            if abs(cpx0f - float(feet_f[0])) > 0.16 or abs(float(v[0])) > 0.50:
                ...
                self.fase, self.t_f, self.calmo = 'recupero', 0.0, 0
            za = 0.72 + (0.78 - 0.72)*smooth(self.t_f/0.8)
            qh, qk = ik(0.0, za)
            feet = 0.5*(d.xpos[BID['r_foot']] + d.xpos[BID['l_foot']])
            dx = com[0] - feet[0] - 0.03
            ap = float(np.clip(-(qh + qk) + np.clip(3.5*dx + 1.2*v[0], -0.4, 0.4),
                               -0.47, 0.47))
            ar = float(np.clip(-(3.5*(com[1]-feet[1]) + 0.7*v[1]), -0.2, 0.2))
            tau = KPB2*(pitch - 0.03) + KDB2*wy   # la fermata coi muscoli del recupero
            for g in ('r', 'l'):
                ch = float(np.clip(self.fprev[AID[g + '_hip_pitch']]
                                   / KP_ATT[g + '_hip_pitch'], -0.6, 0.6))
        ...
        # ---------- VOLANO: spalle + vita ----------
        if self.fase in ('crouch',):
            bersaglio_fw = 0.0
        elif self.fase == 'spinta':
            bersaglio_fw = float(np.clip(-K2F*wy, -0.5, 0.5))
        elif self.fase == 'volo':
            bersaglio_fw = Q_PRONTE
        elif self.fase == 'recupero':
            bersaglio_fw = float(np.clip(K1F*(P0R - pitch) - K2F*wy + Q_PRONTE*0.3,
                                         -2.3, 1.45))
        else:  # ferma: le braccia TORNANO AVANTI = baricentro sui piedi
            bersaglio_fw = -0.35
        veloce = bersaglio_fw > self.q_fw or self.fase == 'volo'
        passo_max = (RATE_ASSORBI if veloce else RATE_RITORNO)*dt
        self.q_fw += float(np.clip(bersaglio_fw - self.q_fw, -passo_max, passo_max))
        ...
        if 'waist_pitch' in AID:
            d.ctrl[AID['waist_pitch']] = (float(np.clip(1.5*(P0R - pitch) - 0.8*wy,
                                                        -0.5, 0.5))
                                          if self.fase in ('recupero', 'ferma')
                                          else 0.0)
        mujoco.mj_step(m, d)
        ...
        if self.fase == 'recupero':
            ...
            serve_passo = abs(cpx0 - float(p_st[0])) > 0.12   # capture fuori dal piede
            # PRIMA la presa, POI il passo: la finestra della vittoria ha precedenza
            presa = (not serve_passo and abs(v[0]) < 0.24 and abs(v[1]) < 0.26
                     and pitch > -0.30)
            self.calmo = self.calmo + 1 if presa else 0
            if self.calmo >= 12 or self.passi >= 8:   # 60 ms di calma sostenuta
                self.fase, self.t_f = 'ferma', 0.0    # frenata riuscita: doppio appoggio
            elif self.t_f >= T_MIN_R and (serve_passo or commit) or self.t_f >= 1.0:
                self.passi += 1
                self.stance = sw
                self.t_f = 0.0
        if d.qpos[2] < 0.40:
...
def campagna():
    print('e5_volano.py - versione ' + VERSIONE)
    print('E5 volano: salto + frustata braccia + passi + fermata.')
    r = prova()
    if r['caduta']:
        print('Volo %.0f ms (+%.1f cm) | passi di recupero %d | poi caduta (attesa).'
              % (r['t_volo_ms'], r['clearance_cm'], r['passi']))
        print('CONQUISTE E5 [C]: la FRUSTATA funziona (40 Nm per tutta la')
        print('corsa, vx -0,67 -> 0 in 80 ms, decollo ripulito fino a -0,06);')
        print('NOVA e stata IN PIEDI PER MEZZO SECONDO dopo il salto (z 0,87,')
        print('busto -3 gradi) prima della deriva. Scoperta 28: le braccia')
        print('lasciate DIETRO dopo la frustata spostano il CoM di -3 cm = la')
        print('deriva che uccide la fermata; fermata e passo si contendono lo')
        print('stato: serve un SUPERVISORE (arbitraggio con isteresi, ev. E6).')
        esito = ('MISURATO (in piedi 0,5 s; resta arbitraggio fermata-passo)'
        ...
        if esito == 'PASS':
            print('*** ATTERRATA IN PIEDI: salto -> %d passi -> FERMA ED ERETTA ***'
                  % r['passi'])
    print('ESITO E5: ' + esito)
    json.dump(dict(versione=VERSIONE, esito=esito, **r),
              open(_OUT('e5_volano.json'), 'w'), indent=1)
    print('Salvato e5_volano.json')
