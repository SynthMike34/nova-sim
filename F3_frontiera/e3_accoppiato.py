...
# -*- coding: utf-8 -*-
"""
e3_accoppiato.py v1.0 - CANTIERE E3 (finale) (fuori campagna, cartella F3 dichiarata):
REGOLATORE ACCOPPIATO x-y-T: trigger laterale a evento (E2, INTATTO) +
controllo DISCRETO della velocita' sagittale: a ogni touchdown il bias di
piazzamento XOFF viene aggiustato di un passetto (integratore sulla mappa di
Poincare'), e v_des sale con rampa solo a ciclo stabilito.
Obiettivo: NOVA cammina IN AVANTI >= 10 passi. Sogno: >= 54.

    python e3_accoppiato.py            demo 3D: marcia e poi spinta avanti (auto)
    python e3_accoppiato.py --test     criterio E1: >= 4 passi senza caduta
...
KI_X = 0.0          # integratore discreto (SPENTO: rompe il ciclo, vedi sweep)
PITCH_REF = 0.16    # inclinazione costante del busto avanti [rad]: la manopola di velocita'
VDES_TGT = 0.20     # velocita' avanti di crociera [m/s]
RAMP0, RAMPN = 6, 10  # la rampa di v_des parte al passo 6 e dura 10 passi
LIFT = 0.06
...
class Camminatrice:
    def __init__(self, v_des=VDES_TGT):
    ...
    def frame(self):
        ...
        vde = self.v_des*float(np.clip((self.passi - RAMP0)/float(RAMPN), 0.0, 1.0))
        cpx = float(com[0] + KCP*v[0]/self.om + KV*(v[0] - vde) + self.xoff_dyn)
        ...
        for gamba in ('r', 'l'):
            if gamba == sw:
                ...
                qh, qk = ik(0.0, ZA0)
                tau = KP_B*(pitch - PITCH_REF) + KD_B*wy
        ...
        if (self.t_f >= T_MIN and commit) or self.t_f >= T_MAX:
            ...
            vx_td = float(v[0])
            self.vx_log.append(round(vx_td, 3))
            self.xoff_dyn = float(np.clip(self.xoff_dyn + KI_X*(vx_td - vde),
                                          -0.16, 0.06))
            self.stance = sw
            self.t_f = 0.0
            self.passi += 1
...
def campagna():
    print('e3_accoppiato.py - versione ' + VERSIONE)
    print('E3 regolatore accoppiato: assetto P0=%.2f come manopola di velocita.' % PITCH_REF)
    print('Criterio: >= 10 passi IN AVANTI (vx > 0).')
    p, tc, x, vlog = prova()
    va = np.array(vlog) if vlog else np.array([0.0])
    vx = float(np.mean(va[-12:])) if len(va) >= 12 else float(va.mean())
    print('Camminata: %d passi | %s | x %+.2f m | vx a regime %+.2f m/s' %
          (p, ('VIVA a fine prova' if tc is None else 'caduta a %.1f s' % tc), x, vx))
    avanti = (x > 0.2 and vx > 0.0 and p >= 10)
    esito = 'PASS' if avanti else 'FAIL'
    print('ESITO E3: %s - NOVA %s' %
          (esito, 'CAMMINA IN AVANTI' if avanti else 'non avanza (vedi handoff)'))
...
def demo():
    ...
    print(__doc__)
    print('E3: NOVA cammina IN AVANTI (P0=%.2f). R = ricomincia.' % PITCH_REF)
    c = Camminatrice()
    caduta_t = None
    def tasto(k):
        if k in (82, 114):
