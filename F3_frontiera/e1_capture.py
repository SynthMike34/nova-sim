...
# -*- coding: utf-8 -*-
"""
e1_capture.py - CANTIERE E1 (fuori campagna, cartella F3 dichiarata):
passi continui col CAPTURE POINT. Il piede di swing insegue in tempo reale
...
T_SW = 0.30         # durata dello swing [s] (sweep: migliore con W=0.18)
LIFT = 0.06
...
class Camminatrice:
    ...
    def frame(self):
        ...
        u = np.clip(self.t_f/T_SW, 0.0, 1.0)
        for gamba in ('r', 'l'):
            if gamba == sw:
...
def prova(spinta=0.3, T=16.0, verbose=False):
    c = Camminatrice()
    dt = m.opt.timestep
    for i in range(int(T/dt)):
        t = i*dt
        if abs(t - 6.0) < dt/2 and spinta:
            c.d.qvel[0] += spinta
        if not c.frame():
            if verbose:
...
def campagna():
    ...
    json.dump(dict(versione=VERSIONE, esito=esito, passi=p, x_m=round(x, 2)),
              open('e1_capture.json', 'w'), indent=1)

def demo():
    import time
    import mujoco.viewer
    print(__doc__)
    print('Record cantiere: 7 passi. R = ricomincia.')
    c = Camminatrice()
    caduta_t = None
    def tasto(k):
        if k in (82, 114):
