...
def campagna():
    ...
    axs[0].axhline(0.88, color='g', ls='--', lw=0.8, label='upright threshold')
    axs[0].set_ylabel('pelvis height [m]')
    axs[0].set_title('F1-D 45 cm chair: seat reaction %.0f N (%.0f%% weight), stand-up %.2f s -> %s'
                     % (r['forza_sedia_N'], r['peso_scaricato_pct'],
                        r['t_alzata_s'] or -1, esito))
    axs[0].legend(fontsize=8)
    axs[1].plot(tr['t'], tr['fs'], color='tab:brown', lw=1.2)
    axs[1].set_ylabel('chair reaction force [N]')
    axs[2].plot(tr['t'], tr['tk'], color='tab:blue', lw=1.2, label='knee')
    axs[2].plot(tr['t'], tr['th'], color='tab:green', lw=1.2, label='hip pitch')
